  ...
  with open(out_path, "w+") as outfile:
    with open(in_csv, newline='') as csvfile:
      ...
      for index, row in enumerate(matchReader):
        line = row[index] + "".join([f'~setLine,{stackindex},{" ".join(stackrow[stackindex:index+1])}' for stackindex, stackrow in enumerate(stack)]) + "~styleWhere,"+row[index]+",color,purple"
